    ...
    def __call__(self, img):
        ...
        if img.ndim == 3:
            ...
            if isinstance(self.size, (int, float)):
                new_size = (int(img.shape[1] * self.size), int(img.shape[0] * self.size))
                return np.array(new_im.resize(new_size, Image.NEAREST))
            else:
                return np.array(new_im.resize(self.size, Image.NEAREST))
            
        elif img.ndim == 2:
            new_im = Image.fromarray(img, mode='F')
            if isinstance(self.size, (int, float)):
                new_size = (int(img.shape[1] * self.size), int(img.shape[0] * self.size))
                return np.array(new_im.resize(new_size, Image.NEAREST))
            else:
